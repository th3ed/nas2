...
Vendored Helm chart for the `news` RAG service: FreshRSS ingest → pgvector → FastMCP server, with an optional web UI for ops + browsing.

## What it ships

- `postgres` — single-pod `ankane/pgvector` StatefulSet, pinned to a node via `nodeSelector`.
- `news-mcp` — FastMCP server exposing `search_articles`, `get_briefing`, `get_article`, `list_recent`, `list_feeds`, `mark_read`, `star`.
- `news-ui` — (opt-in via `ui.enabled`) FastAPI + Jinja2 + HTMX dashboard for sync status and article browsing.
- `ingest` — `CronJob` running the ingest pipeline against FreshRSS + LiteLLM.
- Post-sync migration Job (`migration.enabled`) for clusters with an existing pre-rename `news-rag` namespace.
...
- LiteLLM reachable at `litellm.baseUrl` (default `http://litellm.litellm:4000/v1`) with aliases for the embed and chat models named in `litellm.{embedModel,summarizeModel}`.

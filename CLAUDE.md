...
- **gemma4:e4b emits `reasoning_content` and burns the `max_tokens` budget on chain-of-thought**: gemma4 returns BOTH `choices[0].message.reasoning_content` (CoT trace) and `choices[0].message.content` (the actual answer). With a tight `max_tokens` (e.g. 300), the reasoning eats the whole budget and `content` comes back as an empty string — the request returns HTTP 200 but the answer is silently empty. Symptom: looks like the model "doesn't know" or "refused" but the trace shows it was reasoning fine until cut off. Either bump `max_tokens` (≥1500 for typical news-article summarization), or use the Ollama parameter that disables thinking. See `gitops/manifests/news/chart/templates/ingest-script-configmap.yaml`'s `summarize()` for the worked example (the post-rename path; the parallel `gitops/manifests/news-rag/` copy stays on disk until PR 2 cuts the rollback).

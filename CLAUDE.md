...
| `10` | `alloy`, `agentregistry`, `ollama-embed` |
| `20` | `ollama`, `openclaw` |

GPU workloads (`ollama`, `openclaw`) require `runtimeClassName: nvidia`, which the `gpu-operator` provisions. The `runtimeclass.yaml` lives in `gitops/manifests/gpu-operator/`. `ollama-embed` is deliberately **not** a GPU workload — it serves the embedding model on CPU so the single RTX stays reserved for the chat model (see the GPU-split note in Gotchas).
...
| CPU-only Ollama (Helm release) | `gitops/apps/ollama-embed.yaml` + `gitops/manifests/ollama-embed/values.yaml` |
...
| Upstream-image install (no Helm chart) | `gitops/manifests/honcho/api-deployment.yaml` |
| Hermes `mcp_servers` config block | `gitops/manifests/hermes/configmap.yaml` (mcp_servers.news) |
| Hermes native memory provider | `gitops/manifests/hermes/configmap.yaml` (`memory.provider: honcho` + `honcho.json` sibling key) |
...
- **Local chat model is Qwen3.5 9B (single, GPU-resident)**: `isotnek/qwen3.5:9B-Unsloth-UD-Q4_K_XL` is the one local chat model, exposed via LiteLLM as `qwen3.5:9b` (`ollama_chat/...`). It has reliable **native** tool-calling, so there is **no** LiteLLM `pre_call_hook` and it is **not** in Hermes's `TOOL_USE_ENFORCEMENT_MODELS` tuple (`("gpt","codex","gemini","grok","glm")`) — so Hermes uses its native tool path with no `prompt_builder.py` patch. The old `gemma4:e4b`, `qwen3-coder-next`, and `qwen3:4b-instruct-2507-q8_0` models, the `litellm` `gemma_tool_rewrite` callback (`callbacks-configmap.yaml`), and the hermes `patch-prompt-builder` init container were all removed together. If you add a model whose name contains gpt/codex/gemini/grok/glm, expect Hermes to inject XML tool-enforcement guidance.
- **Qwen3.5 is a thinking model — disable CoT for batch jobs**: like the old gemma4 `reasoning_content` trap, Qwen3.5 can spend a tight `max_tokens` budget on chain-of-thought and return empty `content`. For non-interactive callers (news summarization) pass `"think": false` in the chat/completions body (Ollama-native, passes through LiteLLM's `ollama_chat`) and/or keep `max_tokens` generous. Interactive agents (Hermes/openclaw) leave thinking on for better tool decisions. See `gitops/manifests/news/chart/templates/ingest-script-configmap.yaml`'s `summarize_one()`.
- **If you ever re-add a LiteLLM `pre_call_hook` callback**: a custom callback's `data["model"]` is the user-facing `proxy_config.model_list` entry, **not** the resolved `litellm_params.model`. And the module is loaded by `importlib.util.spec_from_file_location` *relative to `os.path.dirname(config_file_path)`* — for our Helm-mounted config at `/etc/litellm/config.yaml`, mount the callback at `/etc/litellm/<module>.py`, not in site-packages. (The gemma rewrite hook that demonstrated this was deleted with gemma4; check git history at `callbacks-configmap.yaml` for the worked example.)
...
- **Hermes long-term memory is the native Honcho provider** (community pick — Plastic Labs collaborates with Nous Research; first on the official provider list; named most in the Hermes Discord). Wired via `memory.provider: honcho` in `gitops/manifests/hermes/configmap.yaml` plus the sibling `honcho.json` ConfigMap key subPath-mounted at `/opt/data/honcho.json` (subPath mounts freeze at pod-start — `kubectl -n hermes rollout restart deploy/hermes` after edits). Tools surface to the LLM as the canonical built-in `memory_*` names (the ones the model was trained on) AND get the auto-injected `MEMORY_GUIDANCE` block in the system prompt at `run_agent.py:6095` — the combination the old mem0-via-MCP wiring couldn't achieve. Honcho runs as the upstream-published image `ghcr.io/plastic-labs/honcho:v2.0.3` (multi-arch, attested via Plastic Labs' own `.github/workflows/docker-build.yml`); no custom builds, no pip-install initContainer hack. The api server runs with `AUTH_USE_AUTH=false` — network is the auth boundary in-cluster. Honcho's deriver / dialectic / summary / query-gen LLMs all route through our LiteLLM via `LLM_OPENAI_COMPATIBLE_BASE_URL=http://litellm.litellm:4000/v1` + `OPENAI_BASE_URL=...` (belt + suspenders: Honcho's own knob and the openai-python SDK default). Reliability caveat: Honcho's deriver is documented as wanting ≥14B for reliable function calling — Qwen3.5 9B is below that ceiling and produces sparser fact extraction, the same way mem0's deriver did. Accepted ceiling per user direction.
- **Embeddings run on a dedicated CPU-only Ollama (`ollama-embed`), GPU is single-tenant**: the ~10 GB RTX is reserved for the one chat model (Qwen3.5 9B). The embedding model (`nomic-embed-text`, 768-dim) runs on a separate CPU-only Helm release — `gitops/apps/ollama-embed.yaml` + `gitops/manifests/ollama-embed/values.yaml` (`gpu.enabled: false`, no `runtimeClassName`, pinned to nas2). LiteLLM's `text-embedding-3-small` alias points at `http://ollama-embed.ollama:11434`. Consumer today is news ingest (FreshRSS articles → pgvector). Honcho computes embeddings inside its own deriver via the OpenAI-compatible route — same alias, same CPU instance.
- **Cross-node DNS fails for the private `andrews.casa` zone from `desktop`-scheduled pods**: `dig +short freshrss.andrews.casa @10.43.0.10` returns answers from a `nas2`-scheduled pod but empty from a `desktop`-scheduled pod (public domains like `google.com` work fine from both). CoreDNS is on nas2 with `dnsPolicy: Default`, so it uses nas2's resolv.conf (LAN DNS 192.168.111.3 which knows the zone) — yet the cross-node path returns nothing for this zone specifically. Working theory: the desktop node (WSL2) injects an extra `andrews.home` search domain into its pod resolv.conf, and the upstream resolver does something positive-but-empty for queries against `freshrss.andrews.casa.andrews.home` that short-circuits glibc's bare-name retry. Workaround: pin pods that need to reach the `andrews.casa` zone to `nas2` via `nodeSelector: kubernetes.io/hostname: nas2`. See `gitops/manifests/news/chart/values.yaml` (the chart pins the whole `news` stack to nas2 via `nodeSelector`).
...
- **FastMCP behind a TLS-terminating Ingress: clients must use `/mcp`, NOT `/mcp/`** — FastMCP mounts the streamable-http app at `/mcp`; a request to `/mcp/` triggers a Starlette 307 redirect to `/mcp`. The redirect Location is generated from the inbound scheme the pod sees, which is `http://` (the Tailscale Ingress terminates TLS in front of the pod). Modern clients refuse to follow an HTTP redirect when they originally sent an HTTPS request, so the connection looks dead. Symptom in Claude Code: "Unable to connect. Is the computer able to access the url?". Fix is documentation, not a server change: tell every consumer (docs, `~/.claude.json`, `opencode.json`, AgentRegistry catalog) to use `https://<host>/mcp` with no trailing slash. The AgentRegistry catalog URLs are in-cluster `http://...:8080/mcp/` and unaffected, because they're inside the cluster and never traverse the TLS-terminating proxy.

...
- **Embeddings run on a dedicated CPU-only Ollama (`ollama-embed`), GPU is single-tenant**: the ~10 GB RTX is reserved for the one chat model (Qwen3.5 9B). The embedding model (`nomic-embed-text`, 768-dim) runs on a separate CPU-only Helm release — `gitops/apps/ollama-embed.yaml` + `gitops/manifests/ollama-embed/values.yaml` (`gpu.enabled: false`, no `runtimeClassName`, pinned to nas2). LiteLLM's `text-embedding-3-small` alias points at `http://ollama-embed.ollama:11434`. Embedding volume here (news ingest + mem0) is low, so CPU latency is fine. This is why `bge-m3` (1024-dim) was abandoned for GPU use — on first load Ollama sets `batch_size = context_length = 8192` and `cudaMalloc` OOMs when the chat model already pins most of the GPU; on CPU there's no such pressure, but switching off 768-dim would still force a re-embed of all mem0 + news pgvector data, so stay on `nomic-embed-text` unless you migrate dims everywhere.

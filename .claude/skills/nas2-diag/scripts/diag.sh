...
case "$SERVICE" in
  ""|all|ollama|openclaw|caddy|tailscale|docker|gpu|firewall|k3s|argocd|grafana|loki) ;;
  *)
    echo "unknown --service '$SERVICE'. valid: ollama openclaw caddy tailscale docker gpu firewall k3s argocd grafana loki (omit for all)" >&2
    exit 2
    ;;
esac
...
declare -A UNIT=( [ollama]=ollama [openclaw]=openclaw [caddy]=caddy [tailscale]=tailscaled [docker]=docker [k3s]=k3s )
for key in ollama openclaw caddy tailscale docker k3s; do
  want "$key" || continue
...
if want ollama;   then run "docker logs ollama (n=50)"   docker logs --tail 50 ollama   2>&1; fi
if want openclaw; then run "docker logs openclaw (n=50)" docker logs --tail 50 openclaw 2>&1; fi

if want ollama;   then run "curl ollama (host) /api/tags"   curl -fsS  --max-time 5 http://127.0.0.1:11434/api/tags; fi
if want openclaw; then run "curl openclaw (host) UI"        curl -fsSI --max-time 5 http://127.0.0.1:18789/; fi
if want caddy; then
...
if want openclaw; then
  run "openclaw config stat (legacy host)" stat -c '%n  size=%s  mtime=%y  mode=%a  owner=%U' /home/ed/.openclaw/openclaw.json
fi
